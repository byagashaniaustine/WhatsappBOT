...
async def whatsapp_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # --- FIX 1: Extract Primary Phone Number from UNENCRYPTED payload ---
        entry = payload.get("entry", [{}])[0]
        changes = entry.get("changes", [{}])[0]
        value = changes.get("value", {})
        messages = value.get("messages", [])
        contacts = value.get("contacts", []) # Needed later for name lookup
        
        # Safely determine the primary sender number (if a message/contact is present)
        primary_from_number = messages[0].get("from") if messages else None
        ...
        if is_flow_payload:
            try:
                ...
                # --- FIX 2: Inject the Number into Flow Data ---
                        ...
                        if next_screen_key and next_screen_key in FLOW_DEFINITIONS[flow_id_key]:
                            ...
                            if next_screen_key == "LOAN_RESULT" and current_screen == "LOAN_CALCULATOR":
                                ...
                                try:
                                    ...
                                    # FIX 3: Pass user_data twice to correctly trigger calculation in background
                                    background_tasks.add_task(whatsapp_menu, user_data, user_data)
                                    
                                    logger.critical(f"✅ Loan calculation delegated and successful.")
        ...
        # This section uses the already extracted 'primary_from_number'
        
        if messages:
            # Use primary_from_number which is guaranteed to be extracted if messages exist
            from_number = primary_from_number 
            message = messages[0]
